...
class PlayerVehicle extends Vehicle {
  ...
  updatePhysics(tractionMult = 1.0) {
    ...
    if (this.isDashing) {
      ...
      // Apply Speed Multiplier (Yellow Power)
      targetSpeed *= this.speedMult;
      
      // Strict Speed Limit Check (Enforce mass penalties)
      // If our maxSpeed dropped (due to growth), targetSpeed is now lower.
      // We need to ensure we don't stay above it for long.
      targetSpeed = Math.min(targetSpeed, this.maxSpeed * this.speedMult);
    }
